...
''' <summary>
''' 진성대장주 초입 Top3 전략 V2.
''' V1의 기본 구조를 유지하되, 2차 VI 대장주 진입 조건을
''' TickIntensity 단순 임계값이 아니라 TickIntensity5MA 강세 + TickIntensity의 5MA 상승돌파로 판단한다.
'''
''' 핵심:
''' - 현재봉 미확정 지표 사용 금지
''' - 지표 판단은 i-1, 전환/돌파 판단은 i-2 -> i-1 확정봉 기준
''' - 2차 VI 구간: 시가대비 11~16%, TickMA5 > 10, TickIntensity가 TickMA5를 상승돌파
''' </summary>
Public Class TrueLeaderEarlyTrendStrategyV2
...
#Region "Decision"
    Private Function ShouldBuy(ctx As LeaderContext, ByRef reason As String, ByRef signalType As SignalType) As Boolean
        ...
        If ctx.EntrySafetyScore < MinEntrySafetyScore Then
            reason = ctx.BlockReason
            If reason = "" Then reason = String.Format("EntrySafetyScore 부족: {0:0.0}", ctx.EntrySafetyScore)
            Return False
        End If
        ...
        If Not ctx.IsSecondViLeaderEntry AndAlso ctx.RawTickPowerScore < 45.0R Then
        ...
        If ctx.IsSecondViLeaderEntry Then
            reason = String.Format("SecondVI TickCrossUp BuyReady: OpenRise={0:0.00}%, Tick={1:0.00}, PrevTick={2:0.00}, TickMA5={3:0.00}, PrevMA5={4:0.00}, Leader={5:0.0}, Trend={6:0.0}, Safety={7:0.0}",
                                   ctx.OpenRiseRate,
                                   ctx.TickSum,
                                   ctx.PrevTickSum,
                                   ctx.TickMa5,
...
#Region "Context"
    ...
    Private Function IsSecondViLeaderEntryZone(ctx As LeaderContext) As Boolean
        ...
        If Not ctx.TickCrossUpMa5 Then Return False
        Return True
    End Function

    Private Function CalcRawTickPower(ctx As LeaderContext) As Double
        Dim ratioScore As Double = Clamp(ctx.TickRatio / 3.0R * 35.0R, 0.0R, 35.0R)
        Dim crossScore As Double = If(ctx.TickCrossUpMa5, 25.0R, 0.0R)
        Dim slopeScore As Double = If(ctx.TickSlope > 0.0R, 15.0R, 0.0R)
        ...
        Return ratioScore + crossScore + slopeScore + persistScore + turnoverScore
    End Function
    ...
    Private Function CalcTrendStartScore(ctx As LeaderContext) As Double
        ...
        If ctx.TickCrossUpMa5 Then score += 15.0R
    ...
    Private Function CalcEntrySafetyScore(ctx As LeaderContext) As Double
        ...
        If ctx.OpenRiseRate > dynamicMaxOpenRise Then
            ctx.BlockReason = String.Format("시가대비 {0:0.00}%: 초입 매수상한 {1:0.00}% 초과, 2차VI TickCrossUp 조건 미충족(Tick={2:0.00}, TickMA5={3:0.00}, PrevTick={4:0.00}, PrevMA5={5:0.00})",
                                            ctx.OpenRiseRate,
                                            dynamicMaxOpenRise,
                                            ctx.TickSum,
                                            ctx.TickMa5,
